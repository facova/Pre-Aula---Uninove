 ...
 */
class EngineSelectMany implements IEngineWebComponent {

	/** Nome da tag
	 * @var string
	 */
	const TAG_NAME = "selectMany";
	...
	public static function parse(DOMElement $e, BaseModel $model) {
		...
		$select->setAttribute('size',$e->getAttribute('size'));
		...
		if($optNodeList->length > 0) {
			...
			for ($i = 0; $i < $optNodeList->length; $i++ ) {
				...
				if ($opt instanceof DOMElement) {
					if ($opt->localName == EngineSelectOptions::TAG_NAME) {
						EngineSelectOptions::parse($select,$opt,$model,$selected);
					} else if ($opt->localName == "option") {
